...
- general ("G")
- handled ("15", "3,423.10", "5.0E-10")
- headers (i.e. column names)
- headers (that is, as column names)
- hex (255)
- hexadecimal ("X")
- hold (for any type T )
- hour (#duration(0, 1, 0, 0)
...
- input (it will never be called with the default value)
- instead ( '\'  displays \ )
...
- language (informally known as "M")
...
- metadata (rather than merge metadata into possibly existing metadata)
- minute (#duration(0, 0, 1, 0)
- minutes (maximum 59 minutes)
- mm(:ss(.ff))
- month ("M", "m")
- month ("M", "m")
- month ("Y")
...
- numeric ("N")
- objects (for example, dimensions and measures)
- objects (for example, dimensions and measures)
- occur (up to the level where they are handled by a try expression)
- of ()
- offset(s)
- offsets (although this adjustment can be brittle, for example, due to daylight savings time or regional settings)
- one(s)
- online (UTC)
...
- return (first occurrence by default)
- return (U+000D)
- returned (Not a number)
- row(s)
- rows (i.e. the table is empty)
- rows (no duplicates)
